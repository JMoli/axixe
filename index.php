...
<html lang="en">
	...
	<body> <? /*
		...
		*/?>
		<div id="top-message"><?php print $message; ?></div>
		<div id="time"></div>
		...
		<script src="OrbitalControl.js"></script>
		...
		var launch = function(){
				async.parallel([
				function(callback){
					loader.load('triangle.obj', function(obj){
						console.log(obj);
				...
				function(callback){
					loader.load('ae.obj', function(obj){
						...
						_a = obj;
						_a.position.set(-1.75,0,0);
						_a.scale.set(2,2,2);
						_e = obj.clone();
						_e.position.set(1.75, 0 ,0);
						_e.rotation.set(0, Math.PI, 0);
						callback(null);
					});
				},
				function(callback){
					loader.load('xx.obj', function(obj){
						obj.traverse(function(child){
							if(child instanceof THREE.Mesh){
								child.material = new THREE.MeshPhongMaterial({ambient: "#217ABE", color:"#217ABE", specular:"#217ABE", emissive:"#217ABE"});
								if(child.geometry != undefined){
						...
						_x = obj;
						_x.position.set(-0.7,0,0);
						_x.scale.set(2,2,2);
						_amesh.add(_x);
						_xx = obj.clone();
						_xx.position.set(0.7,0,0);
						_amesh.add(_xx);
						callback(null);
					});
				},
				function(callback){
					loader.load('ii.obj', function(obj){
						obj.traverse(function(child){
							if(child instanceof THREE.Mesh){
								child.material = new THREE.MeshPhongMaterial({ambient: "#217ABE", color:"#217ABE", specular:"#217ABE", emissive:"#217ABE"});
								if(child.geometry != undefined){
		...
		launch();
			...
			function render() {
				...
				if(_a.rotation.z >= ((2 * Math.PI) * -2)){
					...
					_a.rotation.z += -0.01953;
					_a.rotation.y += -0.0195;
					_e.rotation.z += 0.01953;
